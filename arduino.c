...
void readPulseSensor()
{
    ...
    {   // 60 hz - or use 33 for two cycles
    ...
    { // main running function
        ...
        if (IR_HFoutput > IR_peak)
            IR_peak = IR_HFoutput;
        if (red_HFoutput > red_Peak)
            red_Peak = red_HFoutput;

        // default reset - only if reset fails to occur for 1800 ms
        if (millis() - lastPeakTime > 1800)
        ...
        if (IR_HFoutput < IR_valley)
            IR_valley = IR_HFoutput;
        if (red_HFoutput < red_valley)
            red_valley = red_HFoutput;
        ...
        if (millis() - lastValleyTime > 1800)
        {                                                                            // insure reset slower than lowest human HB
        ...
        if (lastBinOut == 0 && binOut == 1)
        {
            ...
            struct ld_packet
            {
                ...
                BPM,IR_signalSize,((float)red_baseline / (float)(IR_baseline / 2)), millis()};
